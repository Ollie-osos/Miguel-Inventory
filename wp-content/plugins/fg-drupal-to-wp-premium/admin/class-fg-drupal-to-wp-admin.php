...
if ( !class_exists('FG_Drupal_to_WordPress_Admin', false) ) {
	...
	class FG_Drupal_to_WordPress_Admin extends WP_Importer {
		...
		private function display_admin_page() {
			...
			$data['description'] = __('This plugin will import articles, stories, pages, images, categories and tags from a Drupal database into WordPress.<br />Compatible with Drupal versions 4, 5, 6, 7 and 8.', 'fg-drupal-to-wp');
...
SQL;
		...
		public function get_nodes_count($node_type, $entity_type='node') {
			...
			";
			$sql = apply_filters('fgd2wp_get_nodes_count', $sql, $node_type, $entity_type);
			$result = $this->drupal_query($sql);
		 ...
		 */
		public function convert_taxonomy($taxonomy) {
			$wp_taxonomy = '';
			switch ( $taxonomy ) {
				case 'categories':
		...
		public function import_nodes($content_type, $entity_type='node') {
			...
			$message = sprintf(__('Importing %s...', 'fg-drupal-to-wp'), $plural_content_type_name);
			if ( defined('WP_CLI') ) {
		...
		protected function get_nodes($content_type, $limit=1000, $entity_type = 'node') {
			...
			$sql = "
				SELECT n.nid, n.title, n.type, n.status, n.created
				$extra_cols
		...
		private function get_data_body($node) {
			...
			if ( version_compare($this->drupal_version, '5', '<') ) {
				...
				if ( version_compare($this->drupal_version, '8', '<') ) {
					...
					$extra_join .= "INNER JOIN ${prefix}node n ON n.nid = b.entity_id AND b.language IN(n.language, 'und')";
					$order_by = "ORDER BY b.language";
				} else {
					// Drupal 8
					$table_name = 'node__body';
					$extra_join .= "INNER JOIN ${prefix}node n ON n.nid = b.entity_id AND b.langcode IN(n.langcode, 'und')";
				...
				$extra_join = apply_filters('fgd2wp_get_data_field_extra_join', $extra_join, $node);
		...
		public function convert_node_type($content_type) {
			...
			$post_type = apply_filters('fgd2wp_convert_node_type', $post_type, $content_type);
			return $post_type;
		}
			...
			if ( preg_match_all('#<(img|a)(.*?)(src|href)="(.*?)"(.*?)>#s', $content, $matches, PREG_SET_ORDER) > 0 ) {
				if ( is_array($matches) ) {
					foreach ($matches as $match ) {
						...
						$attachment_id = $this->import_media($image_alt, $filename, $post_date, array('image_caption' => $image_caption), $options);
