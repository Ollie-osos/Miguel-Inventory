...
class FG_Drupal_to_WordPress_Premium_Admin extends FG_Drupal_to_WordPress_Admin {
	...
	public function process_admin_page($data) {
		...
		$data['description'] = __('This plugin will import articles, stories, pages, images, categories, tags, users and comments from a Drupal database into WordPress.<br />Compatible with Drupal versions 4, 5, 6, 7 and 8.', $this->plugin_name);
		...
		foreach ( $images_data as $index => $image_data ) {
			...
			if ( !empty($image) ) {
				...
				if ( preg_match('/^video/', $image['filemime']) ) {
					...
					$replacement = '<img src="' . $filename . '" alt="' . $alt . '"' . $width_assertion . $height_assertion . $style_assertion . ' />';
				}
		...
		if ( isset($custom_field['table_name']) && !empty($custom_field['columns']) ) {
			...
			if ( $this->table_exists($table_name) ) {
				...
				switch ( $field_type ) {
					...
					case 'image':
						$sql = "
							SELECT fm.filename, fm.${uri_field}, ${timestamp_field}, $image_fields
							FROM ${prefix}$table_name f
							$extra_joins
					...
					case 'embed':
						$sql = "
							SELECT fm.filename, fm.${uri_field}, ${timestamp_field}, $file_fields
							FROM ${prefix}$table_name f
							$extra_joins
							INNER JOIN ${prefix}${file_table_name} fm ON fm.fid = ${field_file_id_field}
							WHERE f.${entity_field} = '$entity_id'
	 ...
	 * 
	 * @param string $type Drupal field type
	 * @param string $name Field name
	 * @param string $module Drupal module
	 * @return string Mapped WordPress field type
	 */
	public function map_custom_field_type($type, $name, $module='') {
		...
		if ( version_compare($this->drupal_version, '6', '<') ) {
			...
			if ( !$this->plugin_options['skip_media'] ) {
				...
				foreach ( $attachments as $attachment ) {
					$attachment_id = $this->import_media($attachment['filename'], $attachment['filepath']);
